...
public class EmployeeServiceImpl implements EmployeeService {
    ...
    public EmployeeDTO createEmployee(EmployeeDTO employeeDTO) {
        ...
        if (employeeDTO.getManager() != null &&
                employeeDTO.getManager().getEmployeeId() != null &&
                !employeeRepository.findById(employeeDTO.getManager().getEmployeeId()).isPresent()) {
    ...
    public List<EmployeeDTO> getAllAdmin() {
        ...
        return employeeRepository.findAllByRole("ROLE_ADMIN")
        		.stream()
    ...
    public EmployeeDTO updateEmployee(EmployeeDTO employeeDTO) {
        ...
        if ((employeeDTO.getManager() != null &&
                employeeDTO.getManager().getEmployeeId() != null) &&
                (returnedEmployee.getEmployeeId() == employeeDTO.getManager().getEmployeeId()
                        || !employeeRepository.findById(employeeDTO.getManager().getEmployeeId()).isPresent())) {
        ...
        if (employeeDTO.getManager() != null) {
            returnedEmployee.setManager(EmployeeMapper.mapToEntity(employeeDTO.getManager()));
        } else {
            returnedEmployee.setManager(null);
        }
        return EmployeeMapper.mapToDto(employeeRepository.save(returnedEmployee));
    }
    
    public EmployeeDTO getEmployeeManager(Long id) {
    	Employee managerEmployee = employeeRepository.findByManager(id);
    	return EmployeeMapper.mapToDto(managerEmployee);
    }
    ...
    public List<EmployeeDTO> getAllEmployeeUnderSupervision(Long id) {
        ...
        return employeeRepository.findAllByManager(returnedEmployee)
                .stream()
                .map(employee -> EmployeeMapper.mapToDto(employee))
                .collect(Collectors.toList());
    }
